...
做法：将仓库连接到平台，或直接上传 `dist/` 目录即可。

### 方式 B：自建服务器部署（可选）

将 `dist/` 目录部署到 Nginx / Apache / IIS 等 Web 服务器。

注意：本项目是单页应用（SPA），需要配置“路由回退到 `index.html`”，
否则直接刷新 `/map`、`/logs` 等路径会返回 404。
